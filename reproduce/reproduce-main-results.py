...
def main(frank_result_path, realsumm_result_path):
    ...
    # 评估真实度
    faith_eval = faithfulness_eval(frank_results)
    ...
    # 评估完整度和简洁度
    complete_concise_eval = completeness_and_conciseness_eval(realsumm_results)

    full_eval = {
    "Faithfulness_Evaluation": faith_eval,
    "Completeness_Conciseness_Evaluation": complete_concise_eval
    }
    _model = "Mixtral-8x7B-Instruct-v0.1" # Mixtral-8x7B-Instruct-v0.1  qwen2.5-32b-instruct
    with open(f"./evl/{_model}_full_eval.json", "w") as f:
        json.dump(full_eval, f, indent=4)
...
def faithfulness_eval(results):
    ...
    faith_eval['Sentence-level']['Balanced Accuracy'] = '{:.1%}'.format(bAcc)
...
def completeness_and_conciseness_eval(results):
    ...
    print("\t-Pearson:", pearson_corr)
    print("\t-Spearman:", spearman_corr)
    complete_eval["Summary-level"]['Pearson'] = pearson_corr
    complete_eval['Summary-level']['Spearman'] = spearman_corr
    ...
    print("\t-Rank Correlation:", _rank_correlation)
    complete_eval['System-level']['Rank Correlation'] = _rank_correlation
    ...
    print("\t-Pearson:", pearson_corr)
    print("\t-Spearman:", spearman_corr)
    concise_eval['Summary-level']['Pearson'] = pearson_corr
    concise_eval['Summary-level']['Spearman'] = spearman_corr

    print('* System-level:')
    # model-wise ranking 
    _rank_correlation = rank_correlation(model_wise_results, key="conciseness_scores")
    print("\t-Rank Correlation:", _rank_correlation)
    concise_eval['System-level']['Rank Correlation'] = _rank_correlation


    success_rate = cnt_success_inference / len(conv_ids)
    print('\n* Success ratio', '{:.1%}'.format(success_rate))

    complete_concise_eval = {
        'complete_eval':complete_eval,
        'concise_eval': concise_eval,
        'success_ratio': '{:.1%}'.format(success_rate)
    }
    return complete_concise_eval
...
def get_aggregate_gt_labels(raw_annotations, key):
    ...
    - 用于整合人工标注的GT标签，处理多位标注者的标签不一致问题。通过投票方式确定最终标签，将大多数标注一致的标签作为最终标签。
    '''
...
def balancedAcc(gt, pred):
    ...
    Return:
        - balanced accuracy
    - - 计算二分类任务的平衡准确率，分别计算标签为1和标签为0的准确率，再求其平均值，以避免类别不平衡对准确率的影响
    '''
    ones, zeros = [], []
...
def rank_correlation(model_wise_results, key, min_number=5):
    ...
    - 计算不同模型在给定评价维度（如真实度、完整度、简洁度）上的排名相关系数。
    '''
